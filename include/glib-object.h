...
static void *g_malloc_static(gsize count)
{
 ...
 * @param info Function pointer table used to initialize the vtable of the interface.
 */
static void g_type_add_interface_static(GType instance_type, GType interface_type, const GInterfaceInfo *info)
{
  InterfaceEntry entry = { .type = &g_typePool.classes[interface_type].type, .info = (GInterfaceInfo*)info };
...
#define g_return_val_if_fail(cond, val) \
	if (!(cond))                    \
		return val;
#define G_TYPE_INSTANCE_GET_INTERFACE(obj, type_id, interface_name) private_g_type_get_interface((GTypeInstance*)obj, type_id, sizeof(interface_name))
...
// Originally, this was used internally to check if the instance is of a fundamental type
// However this was added before realizing that a GObject likely derives from GTypeInstance
static bool
